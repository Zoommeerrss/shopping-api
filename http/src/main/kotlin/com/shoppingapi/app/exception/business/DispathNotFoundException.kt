...
class DispathNotFoundException(code: Int, message: String, fields: List<ErrorField>?) : RuntimeException() {
    ...
    constructor(code: Int, message: String, fields: ErrorField) : this(code, message) {
        errorMessage = ErrorMessage(
            status = code,
            message = message,
            timestamp = Date(),
            mutableListOf<ErrorField>(fields)
        )
    }
